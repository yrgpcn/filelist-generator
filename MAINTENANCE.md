...
### 1. 新增功能
- **拖曳选择文件夹**：交互运行时先问"把文件夹拖进窗口"，回车用默认；也支持把文件夹作为位置参数（对应把文件夹拖到 exe / bat 图标上）。可一次给多个文件夹，逐个各生成一份清单。
- **子文件夹统计** 工作表：每个子文件夹的文件数、大小、大小占比，按大小降序，用于快速定位大文件夹。
- **生成后自动打开**（`--open` / `--no-open`）：双击等交互场景默认打开，命令行/管道默认不打开；占用另存时打开的是真正落盘的时间戳文件（`generate_one` 返回实际保存路径）。
- 拖入路径解析用 `parse_drag_paths`：兼容资源管理器投递的带引号含空格路径与手打无引号路径；`filter_existing_dirs` 逐个校验存在性，无效项打印 `[跳过]`。

### 2. 修正：自排除实现曾违反本文件二.1
- 旧实现在绝对路径比对之外，还额外用 `skip_names`（小写文件名集合）排除 `filelist.py` 与 `生成文件清单.bat`——被扫文件夹子目录里的同名文件会被误排除，正是二.1 警示的情形。
