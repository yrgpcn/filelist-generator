...
- **拖曳支持**：将文件夹拖入程序控制台窗口后按回车，或将一个/多个文件夹直接拖到 exe / bat 图标上；多个文件夹时，每个文件夹各自生成一份清单
- **文件清单** 工作表：序号、文件名、扩展名、文件大小、修改日期、所在文件夹、完整路径
- **统计汇总** 工作表：文件总数、文件夹总数、总大小、扩展名分布
- **子文件夹统计** 工作表：每个子文件夹的文件数、占用大小、大小占比，按大小排序，快速定位大文件夹
...
- 可选开关：
  ...
  - 生成后是否自动用 Excel 打开清单（`--open` / `--no-open`；双击运行默认打开，命令行默认不打开）
...
> 💡 **拖曳方式**：将文件夹拖入控制台窗口后按回车，或直接拖到 exe / bat 图标上；一次可拖入多个文件夹，每个文件夹各生成一份清单。

双击运行时会依次询问 4 个问题（直接回车使用默认值）：
1. 文件名要可点击的超链接吗？（默认：是）
...
| `--open` / `--no-open` | 生成后是否自动用 Excel 打开清单（双击默认打开，命令行默认不打开） |
| `文件夹`（位置参数） | 要扫描的文件夹，可指定多个（对应"拖到图标上"的用法）；指定后优先于 `-s` |
