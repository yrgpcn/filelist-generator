...
- **拖曳即用**：把文件夹拖进程序黑色窗口后回车，或直接把一个/多个文件夹拖到 exe / bat 图标上；多个文件夹时每个各生成一份清单
...
> 💡 **拖曳更快**：把文件夹拖进黑色窗口后回车，或直接拖到 exe / bat 图标上；一次拖多个，每个文件夹各生成一份清单。
...
| `文件夹`（位置参数） | 直接跟在命令后要扫描的文件夹，可多个（对应"拖到图标上"）；给了就优先于 `-s` |
